...
function getDirectionText(index) {
    ...
    if (!!resultParent[index - 1]) {
        if (!!resultParent[index + 1]) {
            if (
                resultParent[index + 1].floor - resultParent[index].floor === 0 &&
                resultParent[index].floor === resultParent[index - 1].floor
            ) {
                let curtmpIndex = angleToDirection(
                    util.figureVectorAngle(
                        new THREE.Vector2(
                            resultParent[index + 1].x - resultParent[index].x,
                            resultParent[index + 1].y - resultParent[index].y
                        ),
                        new THREE.Vector2(
                            resultParent[index].x - resultParent[index - 1].x,
                            resultParent[index].y - resultParent[index - 1].y
                        )
                    )
                );
